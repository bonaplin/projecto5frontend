  ...
  return (
    <Draggable draggableId={`${task.id}`} key={`${task.id}`} index={index}>
      {(provided, snapshot) => (
        ...
        >
          <div className="owner text">{task.owner.length > 12 ? task.owner.substring(0, 12) + "..." : task.owner}</div>
          ...
          <div className="category text">{task.category.length > 12 ? task.category.substring(0, 12) + "..." : task.category}</div>
          <div className="title">{task.title.length > 12 ? task.title.substring(0, 12) + "..." : task.title}</div>
